...
def review_phase(phase_id: str):
    ...
    # Amendment system removed - no longer needed
...
def fix_state_corruption() -> bool:
    ...
    try:
        ...
        first_phase_id = phases[0].get("id")
        if not first_phase_id:
            print("   ❌ First phase missing 'id' field")
            return False
        
        reset_phase(first_phase_id)
        print(f"   ✅ Reset to {first_phase_id}")
        return True
...
def main():
    ...
    if command == "discover":
        ...
        print("❌ Amendment system has been removed as part of protocol simplification.")
        print("   Use intent-based validation instead.")
        return 1
...
def handle_amendment_command(args):
    """Handle amendment commands - REMOVED: Amendment system no longer used"""
    print("❌ Amendment system has been removed as part of protocol simplification.")
    print("   Intent-based validation is now used instead of scope amendments.")
    return 1

def handle_patterns_command(args):
    """Handle patterns commands - REMOVED: Pattern learning disabled"""
    print("❌ Pattern learning has been removed as part of protocol simplification.")
    print("   Use deterministic gates (tests, lint, docs) instead.")
